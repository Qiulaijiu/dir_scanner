...
> 整合SRC内部字典和dirsearch通用字典的高性能目录路径探测工具，支持WAF绕过、频率限制、随机UA池
...
| `-i, --include-status`  | 包含的状态码     | 200,201,204,301,302,303,307,308,401,403 |
...
```
默认显示: 200, 201, 204, 301, 302, 303, 307, 308, 401, 403
...
```
  10:30:15  FOUND  [200]     1.2KB    /admin
  10:30:15  REDIR  [301]     0.0B     /old-page
  10:30:16  AUTH   [401]     0.5KB    /api/user
  10:30:16  DENY   [403]     0.3KB    /admin/config
```

| 标签    | 颜色     | 含义                        |
| ------- | -------- | --------------------------- |
| `FOUND` | 绿底黑字 | 成功访问(200/201/204)       |
| `REDIR` | 青底黑字 | 重定向(301/302/303/307/308) |
| `AUTH`  | 黄底黑字 | 需要认证(401)               |
| `DENY`  | 红底黑字 | 禁止访问(403)               |
...
| 输出标签   | FOUND/REDIR/AUTH/DENY分类标签   | 无分类       |
| 报告格式   | 6种                             | 8种          |
| 会话恢复   | 支持                            | 支持         |
| 爬虫功能   | 无                              | 支持         |
...
2. **模式选择** - 默认使用dirsearch通用字典，使用 `--src` 启用SRC专项字典
3. **速率控制** - 建议使用 `--max-rate` 限制请求速率，避免对目标造成过大压力
4. **WAF绕过** - `--waf-bypass` 会增加请求的随机性，但可能降低扫描速度
5. **字典选择** - 使用 `--src-category` 指定相关类别，避免扫描无关路径
